...
#ifndef ALLOC
#  define ALLOC(size) (malloc_c(size))
#endif
#ifndef TRYFREE
#  define TRYFREE(p) {if (p) free_c(p);}
#endif
    ...
    zlib_filefunc64_32_def *pzlib_filefunc64_32_def)
{
	...
	if (pzlib_filefunc64_32_def == NULL)
		return NULL;/* fill_fopen64_filefunc(&ziinit.z_filefunc.zfile_func64); */
    else
    ...
    int strategy, const char *password, uint32_t crc_for_crypting, uint16_t version_madeby, uint16_t flag_base, int zip64)
{
    ...
    if (filename == NULL)
        filename = "-";
    if (comment != NULL)
        size_comment = (uint16_t)strlen_c(comment);

    size_filename = (uint16_t)strlen_c(filename);
    ...
    if ((err == Z_OK) && (password != NULL))
    {
        ...
        if (zi->ci.method == AES_METHOD)
        {
            ...
            fcrypt_init(AES_ENCRYPTIONMODE, (uint8_t *)password, (uint32_t)strlen_c(password), saltvalue, passverify, &zi->ci.aes_ctx);
...
extern int ZEXPORT zipClose2_64(zipFile file, const char *global_comment, uint16_t version_madeby)
{
    ...
    if (global_comment != NULL)
        size_global_comment = (uint16_t)strlen_c(global_comment);
    if (err == ZIP_OK)
